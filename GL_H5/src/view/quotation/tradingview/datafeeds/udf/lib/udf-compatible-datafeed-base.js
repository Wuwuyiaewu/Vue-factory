...
function querySymbolInfo (code_id, store) {
    ...
    return {
        name: code_id,
        // 'exchange-traded': product.szShortName,   不需要这两个字段
        // 'exchange-listed': product.szShortName,   不需要这两个字段
        timezone: 'Etc/UTC', // 交易所时区//Asia/Shanghai//Etc/UTC
        minmov: 1, // 最小波动
        ...
        description: product.aushGB, // 商品说明。这个商品说明将被打印在图表的标题栏中。
        type: 'stock', // 仪表的可选类型 stock, index, forex, futures, bitcoin,  expression,  spread, cfd 或其他字符串
        // supported_resolutions: ['D', '2D', '3D', 'W', '3W', 'M', '6M'],
        pricescale: Math.pow(10, product.uiDigit), // 价格精度
        ticker: code_id,
