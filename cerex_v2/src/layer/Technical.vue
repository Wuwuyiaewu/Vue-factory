        ...
        <li v-for="item of dataList" :key="item.id" class="line_clamp">
          ...
          <div>
            <p>
              ...
              <router-link
                :to="{
                  path: `/learningcentre/technical/detail/${item.id}.html`,
...
export default {
  ...
  methods: {
    setCallBack() {
      ...
      tag.setAttribute(
        "src",
        `https://cerexx.com/fApi/freeDirection?categoryId=743&callback=gotSkill&nocache=${new Date().getTime()}`
      );
  ...
  computed: {
    ...
    dataList() {
      ...
      for (
        let index = 0;
        index < vm.skill_length / vm.fApiValue.PageSize;
        index++
      ) {
        newArr.push(
          vm.skill_list.slice(
      ...
      return newArr[vm.fApiValue.pageIndex - 1];
    },
  },
  watch: {
    // 監聽頁簽變動時，最末與最首鎖定
