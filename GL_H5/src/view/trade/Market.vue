<template>
  ...
  <div>
    ...
    <div class="char_price">
      <ul>
        <li :class="isRise(marketInfo.tick,2,2)" @click="changeDirect(2)">卖出 <em>{{data.ask}}</em><i></i></li>
        ...
        <li :class="isRise(marketInfo.tick,1,2)" @click="changeDirect(1)"><i></i><em>{{data.bid}}</em>买入</li>
      </ul>
    </div>
        ...
        <div class="fixedItem">
          ...
          <div class="itemCon">
            ...
            >
              ...
              <div class="pendTypeBox">
                <p @click="showPendTypeBox=!showPendTypeBox" :class="{'checked': showPendTypeBox}">{{marketType.pendType == 1?"限价":"停损"}} <i></i></p>
                <span :class="errClass.setPriceClass">({{ pendPrice.min }}-{{ pendPrice.max }})</span>
                <ul v-show="showPendTypeBox">
                  <li @click="changePendType(1)" :class="{'checked': marketType.pendType == 1}">限价</li>
                  <li @click="changePendType(2)" :class="{'checked': marketType.pendType == 2}">停损</li>
                </ul>
              </div>

              <div class="volumebox">
                <div class="setbox">
                  <div :class="market.setPrice!=''&&Number(market.setPrice)<=Number(pendPrice.min)?'pic cut stop':'pic cut'" @click="changeSetPrice(1)"/>
                  <input type="number" v-model="market.setPrice" placeholder="未设置">
                  <div :class="market.setPrice!=''&&Number(market.setPrice)>=Number(pendPrice.max)?'pic add stop':'pic add'" @click="changeSetPrice(2)"/>
                </div>
              </div>
            ...
            <!--市价-->
            <div class="item1">
              <p>手数<span :class="errClass.volumeClass">({{ marketInfo.symbolData.minVolumes }}-{{ marketInfo.symbolData.maxVolumes }})</span></p>
              ...
              <div class="volumebox">
                <div class="setbox">
                  ...
                  />
                  <input type="number" v-model="market.volume" placeholder="未设置">
                  <div
                ...
                <div class="quickVolume">
                  <ul>
                    <li 
                    v-for="(vol,index) in setquickvolumes"
                    :key="index"
                    :class="{'checked':market.volume == Number(vol)}"
                    @click="setVolume(vol)"
                    >{{vol}}</li>
                  </ul>
                </div>
            ...
            <!--挂单有效期-->
            <div v-if="isTrue==1" class="item3">
              <p>有效期</p>
              <ul>
                <li @click="setExpireType(2)" :class="{'checked':market.expireType == 2}">当周</li>
                <li @click="setExpireType(1)" :class="{'checked':market.expireType == 1}">当日</li>
              </ul>
            </div>
              ...
              <div class="titShow">
                ...
                >
                  ...
                  <div class="item1">
                    ...
                    <p v-if="marketType.profit == 1">止盈<span :class="errClass.stopProfitClass">({{ stopProfit.min }}-{{ stopProfit.max }})</span></p>
                    <p v-else>止盈<span :class="errClass.stopProfitClass">({{ stopProfit.min }}-{{ stopProfit.max }})</span></p>
            ...
            <div class="item2">
              <ul>
                ...
                <li>
                  <div>
                    <p>可用: $<Available ref="availableChild" /></p>
                  </div>
                </li>
...
export default {
  ...
  computed: {
    ...
    setquickvolumes(){
      return this.marketInfo ? this.marketInfo.symbolData.volumesList : {}
      let minVolumes = this.marketInfo.symbolData.minVolumes
      if (minVolumes){
        return [Number(minVolumes).toFixed(2),Number(minVolumes*2).toFixed(2),Number(minVolumes*10).toFixed(2),Number(minVolumes*20).toFixed(2)]
      }
    },
    /* 挂单价格范围*/
    pendPrice: {
  ...
  data() {
    return {
      ...
      marketType: {
        type: 1, // 市价:1,挂单:2
        direct: this.$route.query.direct||1, // 买:1,卖:2
        profit: 1, // 价格:1,点数:2
  ...
  methods: {
    ...
    setVolume(volume){
      if(volume){
        this.market.volume = Number(volume).toFixed(2)
      }
    },
    resetLoss(){
    ...
    tabName: function(index, item) {
      ...
      if (this.market.stopLoss && this.market.stopLoss != '') {
        if (this.marketType.type == 1) {
          this.market.stopLoss = TradeUtil.changePipValue(this.market.stopLoss, this.marketInfo.tick.ask, this.marketInfo.tick.bid, this.marketInfo.symbolData, this.marketType.direct, this.marketType.profit, 1)
        }else if (this.marketType.type == 2) {
          this.market.stopLoss = TradeUtil.changePipValue(this.market.stopLoss, this.market.setPrice, this.market.setPrice, this.marketInfo.symbolData, this.marketType.direct, this.marketType.profit, 1)
        }
      }
    ...
    isRise: function(obj, directType, type) {
      ...
      if (obj.preData) {
        //console.log("have preData")
        if (Number(obj[direct]) > Number(obj.preData[direct])) {
          val = type == 1 ? this.marketType.direct == directType ? 'rise' : '' : this.marketType.direct == directType ? 'rise bgcolor' : 'rise'
        } else if (Number(obj[direct]) < Number(obj.preData[direct])) {
          val = type == 1 ? this.marketType.direct == directType ? 'fall' : '' : this.marketType.direct == directType ? 'fall bgcolor' : 'fall'
        } else {
          val = type == 1 ? this.marketType.direct == directType ? 'level' : '' : this.marketType.direct == directType ? 'level bgcolor' : 'level'
        }
      } else {
        if (Number(obj[direct]) > Number(obj.open)) {
          val = type == 1 ? 'rise' : this.marketType.direct == directType ? 'rise bgcolor' : 'rise'
        } else if (Number(obj[direct]) < Number(obj.open)) {
          val = type == 1 ? 'fall' : this.marketType.direct == directType ? 'fall bgcolor' : 'fall'
        } else {
          val = type == 1 ? 'level' : this.marketType.direct == directType ? 'level bgcolor' : 'level'
        }
      }
      return val
    },
    ...
    /* 切换限价，停损*/
    changePendType(val) {
      this.showPendTypeBox =! this.showPendTypeBox
      this.marketType.pendType = val
      this.market.stopLoss = ''
      this.market.stopProfit = ''
      if (this.marketType.pendType == 1) { // 限价
        if (this.marketType.direct == 1) this.market.setPrice = this.pendPrice.max// 买
    ...
    submit() {
      ...
      localStorage.setItem('QuoteBuy', JSON.stringify({"code":this.code,"tabIndex":this.$route.query.tabIndex}))
